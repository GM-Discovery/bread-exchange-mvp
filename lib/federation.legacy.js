...
// ---------------------------------------------------------------------------
// v0 Local Commitment Upsert
// - Writes/updates a signed local commitment record for a finalized poll.
// - Purely additive. Does not modify poll semantics.
// ---------------------------------------------------------------------------

function upsertLocalCommitmentForPoll(db, poll, opts = {}) {
  if (!db || !poll) return { ok: false, reason: "missing_inputs" };

  const dataDir = opts.dataDir || require("path").join(__dirname, "..", "data");
  const filePath = require("path").join(dataDir, "federation.local_commitments.json");

  let store = { local: {} };
  try {
    if (require("fs").existsSync(filePath)) {
      store = JSON.parse(require("fs").readFileSync(filePath, "utf8"));
      if (!store || typeof store !== "object") store = { commitments: {} };
  ...
  store.commitments[pollId] = commitment;
