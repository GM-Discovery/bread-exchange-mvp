...
function placeInBracket(poll, queue, bracketSpec, context = {}) {
  ...
  const existingSlot = Number(state.byPollId[poll.id]);
  if (Number.isFinite(existingSlot) && state.bySlot[existingSlot] === poll.id) {
    delete state.bySlot[existingSlot];
  }
  let current = poll;
  let placedSlot = null;
