...
function setDefaults(partial) {
  ...
  const o = Number(partial.opinion_retention_seconds);
  if (Number.isFinite(o) && o > 0) DEFAULTS.OPINION_RETENTION_SECONDS = Math.floor(o);

  const g = Number(partial.governance_retention_seconds);
  if (Number.isFinite(g) && g > 0) DEFAULTS.GOVERNANCE_RETENTION_SECONDS = Math.floor(g);

  const c = Number(partial.governance_cooldown_seconds);
  if (Number.isFinite(c) && c > 0) DEFAULTS.GOVERNANCE_COOLDOWN_SECONDS = Math.floor(c);
}
