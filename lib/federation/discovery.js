...
function incompatibleDiscoveryPayload(partner) {
  return {
    ok: false,
    error: "incompatible_peer",
    ts: nowIso(),
    reason: String(partner.incompatibility_reason || "incompatible_peer"),
    partner: {
      partner_id: String(partner.partner_id || ""),
      exchange_id: String(partner.exchange_id || ""),
      protocol_version: partner.protocol_version || null,
      min_supported_version: partner.min_supported_version || null,
      contract_id: partner.contract_id || null,
      contract_hash: partner.contract_hash || null,
      compatibility_status: partner.compatibility_status || null,
      last_seen_at: partner.last_seen_at || null,
    },
...
function handleDiscovery(req, res, partnersStore, loadDB, opts = {}) {
  ...
  // Partner-gated discovery is blocked if partner is incompatible.
  // Public discovery remains unchanged.
  if (partner) {
    const st = String(partner.compatibility_status || "UNKNOWN");
    if (st !== "OK") {
      return res.status(409).json(incompatibleDiscoveryPayload(partner));
    }
  }
